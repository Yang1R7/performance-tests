...
class DocumentsGatewayHTTPClient(HTTPClient):
    ...
    def get_tariff_document(self, account_id: str) -> GetTariffDocumentResponseSchema:
        """
        Получает документ тарифа для указанного идентификатора счета и парсит его в словарь.

        :param account_id: Идентификатор счета, для которого нужно получить тарифный документ.
    ...
    def get_contract_document(self, account_id: str) -> GetContractDocumentResponseSchema:
        ...
        :param account_id: Идентификатор счета, для которого нужно получить документ контракта.
